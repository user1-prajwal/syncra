...
function injectGlobal() {
  ...
  s.textContent = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @keyframes popIn   { 0%{opacity:0;transform:scale(0.9) translateY(4px)} 100%{opacity:1;transform:scale(1) translateY(0)} }
    .ce-btn-primary { transition: background 0.15s, transform 0.1s, box-shadow 0.15s; }
    .ce-btn-primary:hover { background: #14610a !important; transform: translateY(-1px); box-shadow: 0 4px 8px rgba(0, 0, 0, 0.07); }
    .ce-btn-primary:active { transform: translateY(0); }
    .ce-btn-ghost { transition: border-color 0.15s, color 0.15s; }
    .ce-btn-ghost:hover { border-color: rgba(99,102,241,0.5) !important; color: #F8FAFC !important; }
    .ce-nav-link { transition: color 0.15s; }
    ...
    @media (max-width: 768px) {
      ...
      .hide-mobile { display: none !important; }
      .steps-grid { grid-template-columns: 1fr !important; }
      .ce-mobile-menu-btn { display: flex !important; }
      .cta-visual-panel { flex: 1 1 100% !important; width: 100% !important; min-height: 200px !important; border-right: none !important; border-bottom: 1px solid rgba(99,102,241,0.14) !important; }
    }
    @media (max-width: 420px) {
      .hero-grid { padding-left: 20px !important; padding-right: 20px !important; }
      .join-code-input { width: 96px !important; }
    }
  `;
  document.head.appendChild(s);
}
  ...
  return (
    <nav
      style={{
        ...
        justifyContent: "space-between",
        background:
          pinned || menuOpen ? "rgba(9,9,11,0.92)" : "transparent",
        backdropFilter: pinned || menuOpen ? "blur(12px)" : "none",
        borderBottom:
          pinned || menuOpen
            ? "1px solid rgba(255,255,255,0.06)"
            : "1px solid transparent",
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
        <button
          className="ce-btn-primary hide-mobile"
          onClick={onOpen}
          style={{
            display: "flex",
            alignItems: "center",
        ...
        >
          {menuOpen ? <Ico.Close s={17} /> : <Ico.Menu s={17} />}
        </button>
      </div>
      {menuOpen && (
        <div
...
function Hero({ onOpen }) {
  ...
  className="ce-btn-primary"
  onClick={onOpen}
  // onMouseOver={(e) => (e.currentTarget.style.backgroundColor = "#0B4AD6")} // Darkens slightly on hover
  // onMouseOut={(e) => (e.currentTarget.style.backgroundColor = "#1C5CEF")}  // Resets to original color
  style={{
    display: "flex",
    alignItems: "center",
    gap: "7px",
    ...
    cursor: "pointer",
    fontFamily: "Inter,sans-serif",
    transition: "background-color 0.2s ease", // Makes the hover change smooth
  }}
>
              Create a room
            </button>
            ...
            >
              <input
                className="ce-input join-code-input"
                value={code}
                onChange={(e) => {
                  setCode(
                    e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, ""),
        ...
        >
          ...
          <div>
            ...
            {[
              ...
              {
                href: "https://github.com/user1-prajwal/syncra/issues",
                label: "Report a Bug",
              },
            ].map((l) => (
              <a
